...
class App extends StatelessWidget {
  ...
  late final _router = GoRouter(
    routes: [
      GoRoute(
        path: '/',
        redirect: (_, state) => '/family/${Families.data[0].id}',
      ),
      GoRoute(
        path: '/family/:fid',
        builder: (context, state) => FamilyTabsScreen(
          key: state.pageKey,
          selectedFamily: Families.family(state.pathParameters['fid']!),
        ),
        routes: [
          GoRoute(
            path: 'person/:pid',
            builder: (context, state) {
              final family = Families.family(state.pathParameters['fid']!);
              final person = family.person(state.pathParameters['pid']!);
